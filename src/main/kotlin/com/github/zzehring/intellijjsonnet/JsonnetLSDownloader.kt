...
/**
 * Helper responsible for locating, downloading and updating the jsonnet-language-server binary.
 * It also provides the final executable path for the caller.
 */
class JsonnetLSDownloader(val languageServerRepo: String) {

    private val log = Logger.getInstance(LSPProjectManagerListener::class.java)

    /**
     * Ensures LS binary exists and is up to date, then returns its absolute path.
     */
    fun getOrPrepareExecutable(): String {
        ...
        if (binFile.exists() && upgradeAvailable(binFile, repoInfo.tag)) {
            // show update balloon; user can trigger immediate download
    ...
    // Returns true if a newer version is available compared to the on-disk binary
    ...
    private fun presentUpdateBalloon(
        httpClient: HttpClient,
        binFile: File,
        repoInfo: TargetReleaseInfo
    ) {
        val project = ProjectManager.getInstance().defaultProject
        Notification(
            "lsp",
